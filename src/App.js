  ...
  const saveEntry = useCallback(async (entryData) => {
    if (entryData.id) {
      const { data } = await supabase.from('journal_entries')
        .update({ title: entryData.title, body: entryData.body, mood: entryData.mood, tags: entryData.tags })
        .eq('id', entryData.id).select().single();
      if (data) setEntries(prev => prev.map(e => e.id === data.id ? data : e));
      return data;
    } else {
      const { data } = await supabase.from('journal_entries')
        .insert({ user_id: uid, title: entryData.title, body: entryData.body, mood: entryData.mood, tags: entryData.tags, entry_date: entryData.entry_date })
        .select().single();
...
function JournalPage({ entries, onSave, onDelete }) {
  ...
  const openEntry = e => {
    ...
    setEDate(e.entry_date); setDirty(false);
  };
  ...
  const newEntry = () => {
    ...
    setEDate(todayKey()); setDirty(false);
